...
-- 开始初始化uart
ota_zbuff = zbuff.create(1024)
uart.setup(uart_id, 115200)

-- 几个状态变量, 调试用
local TCP_fota_state = 0
local TCP_rx_counter = 0
local TCP_fota_writed = 0
uart.on(uart_id, "receive", function(id, len)
    -- 防御缓冲区超标的情况
    if ota_zbuff:used() > 8192 then
        log.warn("fota", "ota_zbuff待处理的数据太多了,强制清空")
        ota_zbuff:del()
    end
    while 1 do
        local len = uart.rx(id, ota_zbuff)
        if len <= 0 then
            break
        end
        -- if #s > 0 then -- #s 是取字符串的长度
        TCP_rx_counter = TCP_rx_counter + len
...
sys.taskInit(function()
    ...
    while 1 do
        ...
        local used = ota_zbuff:used()
        if used > 0 then
            if fota_state == 0 then
                -- 等待FOTA的状态
                if used > 5 then
                    local data = ota_zbuff:query()
                    ota_zbuff:del()
                    -- 如果接受到 #FOTA\n 代表数据要来了
                ...
                local result, isDone, cache = fota.run(ota_zbuff)
